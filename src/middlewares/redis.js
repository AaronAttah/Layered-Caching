...
async function initializeRedisClient() {
  ...
  if (redisURL) {
    ...
    redisClient = createClient({ url: redisURL }).on("error", (e) => {
      console.error(`error: Failed to create the Redis client with error:`);
      console.log(`error: Failed to create the Redis client with error:`);
      // console.error(e); 
    });
    ...
    console.error(` Redis client with error: No UrL Provided`);
  }
...
function requestToKey(req) {
  ...
  console.log(reqDataToHash)
  console.log(`${req.path}@${hash.sha1(reqDataToHash)}`)
